...
// GET /api/sync/delta?since=...&ledger_id=...
async function delta(req, res) {
  const { since, ledger_id } = req.query;
  ...
  if (!ledger_id) {
    return res.status(400).json({ error: 'ledger_id 파라미터가 필요합니다' });
  }

  try {
    // 소유자 또는 공유받은 사용자인지 확인
...
async function push(req, res) {
  const { transactions = [], categories = [], ledger_id } = req.body;
  const userId = req.user.user_id;

  if (!ledger_id) {
    return res.status(400).json({ error: 'ledger_id가 필요합니다' });
  }

  try {
    // 소유자 또는 edit 권한 공유자인지 확인
    ...
    for (const cat of categories) {
      ...
      if (is_deleted && server_id) {
        ...
        const r = await pool.query(
          ...
          [ledger_id, category_name, color ?? null, categoryType]
        );
        catResults.push({
          client_id,
    ...
    for (const tx of transactions) {
      ...
      if (is_deleted && server_id) {
        ...
        const r = await pool.query(
          ...
          [ledger_id, userId, amount, type, category_server_id ?? null,
